  ...
  adjust(){
    ...
    cleanSvgPoints.forEach((points,idx)=>{
      ...
      for(let i = 0;i<points.length - 1;i++){
        ...
        const midRealAngle = Math.atan2(p2.y - p1.y,p2.x - p1.y);

        const [p11,p21] = getCornersCoordinate(p1.x,p1.y,p1.realAngle!,10);
        const [p12,p22] = getCornersCoordinate(p2.x,p2.y,p2.realAngle!,10);
        const [pm1,pm2] = getCornersCoordinate(pm.x,pm.y,midRealAngle,10);

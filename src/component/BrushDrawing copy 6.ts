...
export default class Eraser{
  ...
  adjust(){
    type PointType = {x:number,y:number,d:number,angle1:number,angle2:number,distance:number};
    const delta = 5;
    ...
    const getQuadraticCurveControl = (p1:PointType,p2:PointType) => {
      const x1 = p1.x;
      const y1 = p1.y;
      const firstAngle = p1.angle2;
      const x2 = p2.x;
      const y2 = p2.y;
      const secondAngle = p2.angle1;

      let targetX:number;
      let targetY:number;

      if((firstAngle === 90 || firstAngle === -90) && (secondAngle === 90 || secondAngle === -90) ){
        targetX = (x1 + x2)/2;
        targetY = (y1 + y2)/2;
      }else if(firstAngle === 90 || firstAngle === -90){
        const k2 = Math.atan(secondAngle / 180 * Math.PI);
        const b2 = y2 - k2 * x2;
        targetX = x1;
        targetY = k2 * targetX + b2; 
      }else if(secondAngle === 90 || secondAngle === -90){
        const k1 = Math.atan(firstAngle / 180 * Math.PI);
        const b1 = y1 - k1 * x1;
        targetX = x2;

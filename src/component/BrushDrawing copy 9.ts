  ...
  adjust(){
    ...
    cleanSvgPoints.forEach((points,idx)=>{
      ...
      for(let i = 0;i<points.length - 1;i++){
        ...
        const avgD = getAvgD(currentGroup,p1,p2);
        const midRealAngle = Math.atan2(p2.y - p1.y,p2.x - p1.x);

        const [p11,p21] = getCornersCoordinate(p1.x,p1.y,p1.realAngle!,30);
        const [p12,p22] = getCornersCoordinate(p2.x,p2.y,p2.realAngle!,30);
        const [pm1,pm2] = getCornersCoordinate(pm.x,pm.y,midRealAngle,30);

        const [pm01,pm02] = getCornersCoordinate(pm.x,pm.y,midRealAngle,0);

        const [c11,c12] = getBezierCurveControls(p11,p12,pm1);
        const [c21,c22] = getBezierCurveControls(p21,p22,pm2);
        ...
        // this.ctx.arc(pm1.x,pm1.y,2,0,Math.PI * 2);
        // this.ctx.fill();
        // this.ctx.restore();

        if(!i){
          pathStr0 = `M${p1.x},${p1.y}C${pm01.x},${pm01.y},${pm02.x},${pm02.y},${p2.x},${p2.y}`;

          pathStr = `M${p11.x},${p11.y}C${c11.x},${c11.y},${c12.x},${c12.y},${p12.x},${p12.y}`;

          pathStr2 = `M${p21.x},${p21.y}C${c21.x},${c21.y},${c22.x},${c22.y},${p22.x},${p22.y}`;
        }else{
          pathStr0 += `C${pm01.x},${pm01.y},${pm02.x},${pm02.y},${p2.x},${p2.y}`;

          pathStr += `C${c11.x},${c11.y},${c12.x},${c12.y},${p12.x},${p12.y}`;

          pathStr2 += `C${c21.x},${c21.y},${c22.x},${c22.y},${p22.x},${p22.y}`;
        }
      ...
      const path2 = new Path2D(pathStr2);
